...
{
    private class ListBuildings
    {
        public int id, metal, crystal, deuter, income, capacity, level;
        public string name, description;
        public float factor;

        public ListBuildings(int i, string n, string de, int m, int c, int d, int inc, int ca, float fa, int l)
        {
            this.id = i;
            this.name = n;
            this.description = de;
    ...
    private List<ListBuildings> buildings = new List<ListBuildings>();
    ...
    private void Start()
    {
        ...
        buildings.Add(new ListBuildings(0, "METAL MINE", "Each level of metal mine gives +5/30 sec income metal.", 24, 13, 0, 5, 0, 1.2f, 1));
        buildings.Add(new ListBuildings(1, "CRYSTAL MINE", "Each level of crystal mine gives +5/30 sec income crystal.", 26, 17, 0, 5, 0, 1.2f, 1));
        buildings.Add(new ListBuildings(2, "DEUTER SINTETIZER", "Each level of deuter sintetizer gives +5/30 sec income deuter.", 31, 23, 0, 5, 0, 1.2f, 1));
        buildings.Add(new ListBuildings(3, "LABORATORY", "Each level laboratory gives more researches to learn.", 75, 100, 45, 0, 0, 1.8f, 1));
        buildings.Add(new ListBuildings(4, "HANGAR", "Each level of hangar gives more ships and lasers to buy.", 100, 50, 75, 0, 0, 1.8f, 1));
        buildings.Add(new ListBuildings(5, "METAL STORE", "Each level of metal store gives +1000 free space of magazine.", 100, 0, 0, 0, 1000, 1.5f, 1));
        buildings.Add(new ListBuildings(6, "CRYSTAL STORE", "Each level of crystal store gives +1000 free space of magazine.", 75, 50, 0, 0, 1000, 1.5f, 1));
        buildings.Add(new ListBuildings(7, "DEUTER STORE", "Each level of deuter store gives +1000 free space of magazine.", 75, 50, 50, 0, 1000, 1.5f, 1));
        buildings.Add(new ListBuildings(8, "TERRAFORMER", "Each level of terraformer gives +15 free space of planet to build new buildings.", 0, 450, 600, 0, 0, 1.5f, 1));
        CheckButtons();
    }
    ...
    private void SetFreeField(int value)
    {
    ...
    {
        ShowInformation(nr, buildings[nr].description);
    }
